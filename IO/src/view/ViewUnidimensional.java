...
public class ViewUnidimensional extends JPanel {
	...
	public ViewUnidimensional(){
		...
		JLabel lblNewLabel_1 = new JLabel(" (1-2*x + 0.05*sen(4*pi*x)-x)² + (x -0.5*sen(2*pi*x))²");
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource() == minimizar) {
				if(biseccionCheckBox.isSelected()) {
					...
					maxIterations = Integer.parseInt(maxIterationsField.getText());
					String text = "EL METODO NO FUNCIONA ADECUADAMENTE PARA TODOS LOS VALORES \n \n";
					text += NewtonUnidimensional.minimiza(f, x,maxIterations);
					textArea.setText(text);
				}
			}
			else if(e.getSource() == dibujar)
				Branin.dibujaUnidimensional(f);
		}
	}
